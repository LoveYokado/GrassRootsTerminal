...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setLanguage(langMode) {
        const currentModeMatch = document.cookie.match(/menu_mode=(\d+)/);
        const currentMode = currentModeMatch ? currentModeMatch[1] : '1';

        if (currentMode === langMode) return;

        const confirmMsg = textData?.terminal_ui?.settings_popup?.language_change_confirm || "Changing language requires a page reload.\nAre you sure?";
        if (confirm(confirmMsg)) {
            document.cookie = `menu_mode=${langMode}; path=/; max-age=31536000`;
            window.location.reload();
        }
    }

    if (langBtnEn) langBtnEn.addEventListener('click', () => setLanguage('1'));
    if (langBtnJa) langBtnJa.addEventListener('click', () => setLanguage('2'));
